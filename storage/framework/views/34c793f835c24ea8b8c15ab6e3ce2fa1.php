        ...
        <div class="bg-green-50 p-4 rounded-lg border border-green-200">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-green-900"><?php echo e(number_format($data['active'] ?? 0)); ?></p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-red-50 p-4 rounded-lg border border-red-200">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-red-900"><?php echo e(number_format($data['inactive'] ?? 0)); ?></p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-purple-50 p-4 rounded-lg border border-purple-200">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-purple-900"><?php echo e(number_format($data['new_this_month'] ?? 0)); ?></p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="bg-gray-50 p-4 rounded-lg">
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        <span class="w-3 h-3 rounded-full mr-2" style="background-color: <?php echo e($role['color']); ?>"></span>
                        <span class="text-sm text-gray-600"><?php echo e($role['name']); ?></span>
                    </div>
                    <div class="flex items-center">
                        <div class="w-32 bg-gray-200 rounded-full h-2 mr-3">
                            <div class="h-2 rounded-full" style="width: <?php echo e($role['percentage']); ?>%; background-color: <?php echo e($role['color']); ?>"></div>
                        </div>
                        <span class="text-sm font-medium text-gray-800"><?php echo e(number_format($role['count'])); ?> (<?php echo e($role['percentage']); ?>%)</span>
                    </div>
                </div>
        ...
        <div class="bg-gray-50 p-4 rounded-lg">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-lg font-semibold text-green-600"><?php echo e(number_format($data['logged_in_today'] ?? 0)); ?></span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">User Login Minggu Ini</span>
                    <span class="text-lg font-semibold text-blue-600"><?php echo e(number_format($data['logged_in_this_week'] ?? 0)); ?></span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">User Login Bulan Ini</span>
                    <span class="text-lg font-semibold text-purple-600"><?php echo e(number_format($data['logged_in_this_month'] ?? 0)); ?></span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Rata-rata Login per Hari</span>
                    <span class="text-lg font-semibold text-gray-800"><?php echo e(number_format($data['avg_daily_logins'] ?? 0)); ?></span>
                </div>
                <div class="pt-2 border-t">
                    <div class="text-sm text-gray-600 mb-2">Tingkat Aktivitas User:</div>
                    <div class="w-full bg-gray-200 rounded-full h-3">
                        <div class="bg-green-500 h-3 rounded-full" style="width: <?php echo e($data['activity_rate'] ?? 0); ?>%"></div>
                    </div>
                    <div class="text-center text-sm font-medium text-gray-800 mt-1"><?php echo e($data['activity_rate'] ?? '0'); ?>%</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="bg-gray-50 p-4 rounded-lg">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-lg font-semibold text-blue-600"><?php echo e(number_format($data['male_users'] ?? 0)); ?> (<?php echo e($data['male_percentage'] ?? '0'); ?>%)</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Perempuan</span>
                    <span class="text-lg font-semibold text-pink-600"><?php echo e(number_format($data['female_users'] ?? 0)); ?> (<?php echo e($data['female_percentage'] ?? '0'); ?>%)</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Tidak Diketahui</span>
                    <span class="text-lg font-semibold text-gray-600"><?php echo e(number_format($data['unknown_gender'] ?? 0)); ?> (<?php echo e($data['unknown_percentage'] ?? '0'); ?>%)</span>
                </div>
                <div class="pt-2 border-t">
                    <div class="text-sm text-gray-600 mb-2">Rasio Gender (L:P):</div>
                    <div class="text-center text-lg font-semibold text-gray-800">
                        <?php if(($data['female_users'] ?? 0) > 0): ?>
                            <?php echo e(number_format(($data['male_users'] ?? 0) / ($data['female_users'] ?? 1), 2)); ?> : 1
                        <?php else: ?>
                            N/A
                        <?php endif; ?>
                    </div>
    ...
    <div class="bg-gray-50 p-4 rounded-lg mb-6">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full">
                ...
                <tbody>
                    <?php $__currentLoopData = $data['most_active_users'] ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr class="border-b border-gray-100">
                        <td class="py-2 px-3 text-sm text-gray-800"><?php echo e($user['name']); ?></td>
                        <td class="py-2 px-3 text-sm text-gray-600">
                            <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs"><?php echo e($user['role']); ?></span>
                        </td>
                        <td class="py-2 px-3 text-sm text-gray-600"><?php echo e($user['last_login']); ?></td>
                        <td class="py-2 px-3 text-sm text-gray-600"><?php echo e(number_format($user['login_count'])); ?></td>
                        <td class="py-2 px-3 text-sm">
                            <div class="flex items-center">
                                <div class="w-16 bg-gray-200 rounded-full h-2 mr-2">
                                    <div class="bg-green-500 h-2 rounded-full" style="width: <?php echo e($user['activity_score']); ?>%"></div>
                                </div>
                                <span class="text-xs text-gray-600"><?php echo e($user['activity_score']); ?>%</span>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
if (document.getElementById('userRoleChart')) {
    ...
    new Chart(ctx, {
        type: 'doughnut',
        data: {
            labels: <?php echo json_encode(array_column($data['roles'] ?? [], 'name')); ?>,
            datasets: [{
                data: <?php echo json_encode(array_column($data['roles'] ?? [], 'count')); ?>,
                backgroundColor: <?php echo json_encode(array_column($data['roles'] ?? [], 'color')); ?>
...
if (document.getElementById('userMonthlyChart')) {
    ...
    new Chart(ctx, {
        type: 'bar',
        data: {
            labels: <?php echo json_encode(array_column($data['monthly_registrations'] ?? [], 'month')); ?>,
            datasets: [{
                label: 'Registrasi Bulanan',
                data: <?php echo json_encode(array_column($data['monthly_registrations'] ?? [], 'count')); ?>,
...
</script><?php /**PATH D:\laragon\www\kantor-camat-waesama\resources\views\admin\laporan\users.blade.php ENDPATH**/ ?>